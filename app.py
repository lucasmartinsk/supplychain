...
# --- DATABASE LOGIC (All in one file to avoid import errors) ---
...
st.write(
    "Manage vendors, track missing documentation, and map third-party supply"
    " chain risks."
)
...
menu = st.sidebar.selectbox(
    ...
    [
        "Dashboard & Import",
        "Vendor Directory",
        "Subcontractor Chain (MOVEit Analysis)",
    ],
)
...
if menu == "Dashboard & Import":
  ...
  st.info(
      "Upload your structured Excel file containing the sheets: `vendors`,"
  ...
  if uploaded_file is not None:
    try:
      ...
      if st.button("Commit Data to Database"):
        ...
        st.success(
            "✅ All tables were successfully imported and saved to the database!"
        )

    except Exception as e:
      st.error(
          "Error processing the file. Ensure the sheets are named correctly"
          f" (`vendors`, `documents`, `subcontractors`). Details: {e}"
      )
  ...
  if not df_vendors.empty:
    criticality_filter = st.selectbox(
        "Filter by Criticality", ["All"] + list(df_vendors["criticality"].unique())
    )
    ...
    if not df_docs.empty:
      df_merged = df_docs.merge(
          df_vendors[["vendor_id", "name"]], on="vendor_id", how="left"
      )
      st.dataframe(
          df_merged[["name", "doc_type", "status", "expiry_date"]],
    ...
    st.warning(
        "No vendors found. Please upload your Excel file in the 'Dashboard &"
  ...
  st.write(
      "Critical analysis focused on uncovering hidden subcontractors (Lessons"
      " learned from the MOVEit supply chain incident)."
  )
  ...
  if not df_subs.empty and not df_vendors.empty:
    df_sub_merged = df_subs.merge(
        df_vendors[["vendor_id", "name"]],
    ...
    st.warning(
        "⚠️ Vendors with **undisclosed** subcontractors represent high"
        " compliance risks and hidden shadow IT vectors."
    )

    for index, row in df_sub_merged.iterrows():
      disclosure_status = (
          "🟢 Disclosed"
          if row["disclosed_by_vendor"]
          else "🔴 Hidden / Discovered Internally"
      )
      with st.expander(
          f"Parent Vendor: {row['name']} ➔ Subcontractor:"
          f" {row['subcontractor_name']} ({disclosure_status})"
      ):
        st.write(f"**Service Provided:** {row['service_provided']}")
        st.write(
            "**Disclosed by Primary Vendor?**"
            f" {row['disclosed_by_vendor']}"
        )
  else:
